...
public class DungeonGeneratorToBSP1 : NetworkBehaviour
{
    [SerializeField]
    ...
    List<Matrix4x4> wallMatrixs = new List<Matrix4x4>();
    ...
    void CombineMesh()
    {
        MeshFilter[] filters = dungeonRoot.GetComponentsInChildren<MeshFilter>();
        ...
        dungeonRoot.GetComponent<MeshFilter>().mesh = combinedMesh;
    }
...
void CarveCorridor(Vector2Int from, Vector2Int to, Vector2Int exitDir)
{
    ...
    void RenderWallInstances()
    {
        int batchSize = 1023;

        for(int i = 0; i < wallMatrixs.Count; i += batchSize)
        {
            int count = Mathf.Min(batchSize, wallMatrixs.Count - i);

            Graphics.DrawMeshInstanced(
                wallMesh, 
                0,
                wallMaterial,
                wallMatrixs.GetRange(i, count));
        }
    }
    ...
    public void BuildRoomControllers()
    {
        foreach(var room in rooms)
        {
            //Debug.Log("룸 생성");

            GameObject go = Instantiate(dungeonController);
            ...
            DungeonController rc = go.GetComponent<DungeonController>();
    ...
    void BuildSpawnPoints(Room room, DungeonController rc)
    {
        ...
        int xMin = room.rect.xMin + 5;
        int xMax = room.rect.xMax - 5;
        int yMin = room.rect.yMin + 5;
        int yMax = room.rect.yMax - 5;
    ...
    public void BuildDoorsFromRequests()
    {
